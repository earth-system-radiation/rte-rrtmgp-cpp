...
template<typename Float> __device__ constexpr Float k_min();
...
void lw_transport_noscat_kernel(
        const int icol, const int ncol, const int nlay, const int ngpt, const Bool top_at_1,
        ...
        Float* __restrict__ source_sfc, Float* __restrict__ sfc_albedo, Float* __restrict__ source_sfc_jac)
{
    ...
    {
        const int idx = icol;
        sfc_albedo[idx] = Float(1.) - sfc_emis[idx];
        ...
        Float* __restrict__ albedo, Float* __restrict__ src, Float* __restrict__ denom)
{
    ...
    if ( (icol < ncol) )
    {
        if (top_at_1)
        {
            const int sfc_idx_3d = icol;
            const int sfc_idx_2d = icol;
            ...
            {
                const int lay_idx  = icol + ilay*ncol;
                const int lev_idx1 = icol + ilay*ncol;
                const int lev_idx2 = icol + (ilay+1)*ncol;
            ...
            for (int ilay=1; ilay < (nlay+2); ++ilay)
            {
                const int lev_idx1 = icol + ilay*ncol;
                const int lev_idx2 = icol + (ilay-1)*ncol;
                const int lay_idx = icol + (ilay-1)*ncol;
                if (ilay < (nlay+1)) {
        ...
        else
        {
            ...
            const int top_idx = icol + nlay*ncol;
            flux_up[top_idx] = flux_dn[top_idx] *albedo[top_idx] + src[top_idx];
...
*/
// __global__
// void lw_solver_noscat_gaussquad_kernel(const int ncol, const int nlay, const int ngpt, const Float eps,
...
template<typename Float> __device__ constexpr Float tmin();
...
template<typename Float> __device__ constexpr Float tkmin();

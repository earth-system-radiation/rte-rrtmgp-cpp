    ...
    if (input_nc.variable_exists(vmr_gas_name))
    {
        ...
        else if (n_dims == 3)
        {
            if (dims.at("lay") == n_lay && dims.at("x") == n_col_x && dims.at("y") == n_col_y)
                gas_concs.set_vmr(gas_name,
                        Array<TF,2>(input_nc.get_variable<TF>(vmr_gas_name, {n_lay, n_col_y, n_col_x}), {n_col_x * n_col_y, n_lay}));
            else
                throw std::runtime_error("Illegal dimensions of gas \"" + gas_name + "\" in input");
        }
    }
...
void solve_radiation(int argc, char** argv)
{
    ...
    Array<TF,2> p_lay(input_nc.get_variable<TF>("p_lay", {n_lay, n_col_y, n_col_x}), {n_col, n_lay});
    Array<TF,2> t_lay(input_nc.get_variable<TF>("t_lay", {n_lay, n_col_y, n_col_x}), {n_col, n_lay});
    Array<TF,2> p_lev(input_nc.get_variable<TF>("p_lev", {n_lev, n_col_y, n_col_x}), {n_col, n_lev});
    Array<TF,2> t_lev(input_nc.get_variable<TF>("t_lev", {n_lev, n_col_y, n_col_x}), {n_col, n_lev});
    ...
    if (input_nc.variable_exists("col_dry"))
    {
        ...
        col_dry = std::move(input_nc.get_variable<TF>("col_dry", {n_lay, n_col_y, n_col_x}));
    }
    ...
    {
        lwp.set_dims({n_col_x, n_col});
        lwp = std::move(input_nc.get_variable<TF>("lwp", {n_lay, n_col_y, n_col_x}));

        iwp.set_dims({n_col_x, n_col});
        iwp = std::move(input_nc.get_variable<TF>("iwp", {n_lay, n_col_y, n_col_x}));

        rel.set_dims({n_col_x, n_col});
        rel = std::move(input_nc.get_variable<TF>("rel", {n_lay, n_col_y, n_col_x}));

        rei.set_dims({n_col_x, n_col});
        rei = std::move(input_nc.get_variable<TF>("rei", {n_lay, n_col_y, n_col_x}));
    }
    ...
    if (switch_longwave)
    {
        ...
        Array<TF,2> emis_sfc(input_nc.get_variable<TF>("emis_sfc", {n_col_y, n_col_x, n_bnd_lw}), {n_bnd_lw, n_col});
        Array<TF,1> t_sfc(input_nc.get_variable<TF>("t_sfc", {n_col_y, n_col_x}), {n_col});
    ...
    if (switch_shortwave)
    {
        ...
        Array<TF,1> mu0(input_nc.get_variable<TF>("mu0", {n_col_y, n_col_x}), {n_col});
        Array<TF,2> sfc_alb_dir(input_nc.get_variable<TF>("sfc_alb_dir", {n_col_y, n_col_x, n_bnd_sw}), {n_bnd_sw, n_col});
        Array<TF,2> sfc_alb_dif(input_nc.get_variable<TF>("sfc_alb_dif", {n_col_y, n_col_x, n_bnd_sw}), {n_bnd_sw, n_col});

        Array<TF,1> tsi_scaling({n_col});
        if (input_nc.variable_exists("tsi"))
        {
            Array<TF,1> tsi(input_nc.get_variable<TF>("tsi", {n_col_y, n_col_x}), {n_col});

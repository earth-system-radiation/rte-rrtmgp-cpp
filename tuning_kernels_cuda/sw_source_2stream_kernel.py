...
def run_and_test(params: dict):
    ...
    two_stream_result = kt.run_kernel(
            ...
            two_stream_args, dict(RTE_RRTMGP_USE_CBOOL=1, block_size_x=64, block_size_y=1, block_size_z=1), compiler_options=cp)
    ...
    fused_result = kt.run_kernel(
            ...
            fused_kernel_args, dict(RTE_RRTMGP_USE_CBOOL=1, block_size_x=16, block_size_y=16), compiler_options=cp)
...
def tune_fused_kernel():
    ...
    fused_result = kt.tune_kernel(
            fused_kernel_name, kernels_src, ref_problem_size,
            fused_kernel_args, tune_params, compiler_options=cp,
...
def tune_two_stream_ref():
    ...
    kt.tune_kernel(
            two_stream_ref_kernel_name, ref_kernels_src, two_stream_ref_problem_size,
            two_stream_args, tune_params, compiler_options=cp)
...
if __name__ == '__main__':
    ...
    tau  = np.random.random(opt_size).astype(type_float)
    ssa  = np.random.random(opt_size).astype(type_float)
    g = np.random.random(opt_size).astype(type_float)
    mu0 = np.random.random(ncol).astype(type_float)
    ...
    flux_dir = 1+np.random.random(flx_size).astype(type_float)
    ...
    source_kernel_name = f'sw_source_kernel_top_at_1<{str_float}, {top_at_1}, {nlay}>'
    source_ref_kernel_name = f'sw_source_kernel<{str_float}>'

    two_stream_ref_problem_size = (ncol, nlay, ngpt)
    two_stream_ref_kernel_name = f'sw_2stream_kernel<{str_float}>'
    two_stream_kernel_name = f'sw_2stream_kernel<{str_float}>'

    fused_kernel_name = f"sw_source_2stream_kernel<{str_float}, {top_at_1}>"
